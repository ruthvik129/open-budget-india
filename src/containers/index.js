...
const Main = () => {
  ...
  const formFacetURL = () => {
    let formFacetPair = `${filterType}:${facetValue}`;
    if (facetValueArray.length !== 0) {
  ...
  const formDatasetFetchUrl = () => {
    ...
    switch (userAction) {
      case "search":
        url = `https://openbudgetsindia.org/api/3/action/package_search?q=${value}&start=${
          activePageNumber * PAGE_COUNT
        }&rows=${PAGE_COUNT}`;
        break;

      case "facet":
        url = `https://openbudgetsindia.org/api/3/action/package_search?fq=${formFacetURL()}&start=${
          activePageNumber * PAGE_COUNT
        }&rows=${PAGE_COUNT}`;
        break;

      default:
        url = `https://openbudgetsindia.org/api/3/action/package_search?start=${
          activePageNumber * PAGE_COUNT
        }&rows=${PAGE_COUNT}`;
    }
    return url;
  ...
  useEffect(() => {
    let url = `https://openbudgetsindia.org/api/3/action/package_search?facet.field=["tags","groups","organization"]&facet.limit=10&rows=0`;

    if (userAction === "facet") {
      url = `https://openbudgetsindia.org/api/3/action/package_search?facet.field=["tags","groups","organization"]&fq=${formFacetURL()}&facet.limit=10&rows=0`;
    }
  ...
  const removeFilter = (selectedFilter) => {
    ...
    facetValueFilters.length === 0 && setuserAction("");
  };
